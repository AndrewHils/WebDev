...
// Добавляем обработчик события на клик
    clearButton.addEventListener('click', function() {
        ...
        // Створюємо новий об'єкт викладача
        const newTeacher = {
            full_name: document.getElementById('teacher-name').value,
            course: document.getElementById('teacher-specialty').value,
            ...
            age: (function() {
                const dateOfBirth = document.getElementById('teacher-dob').value
                // Перетворення дати народження на об'єкт Date
            ...
            note: document.getElementById('teacher-note').value
        };

        console.log("len1: " + window.allTeachers.length);
        window.allTeachers.push(newTeacher)
        console.log("le2n: " + window.allTeachers.length);
...
function closePopup() {
    ...
    window.displayFilterTeachers()
}

function closeInfo() {
    let overlay = document.querySelector(".popup-overlay-info");

    if (overlay && overlay.parentNode) {
        overlay.classList.replace('popup-overlay-info', 'popup-overlay-info-hide');
